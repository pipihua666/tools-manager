...
import { listPresets, applyPreset, movePreset, moveSkillPreset, removeSkillFromPreset, syncSkill, type SyncMode } from "./core/preset";
import { addMcpServer, getMcpServer, importMcpFromTools, listMcpServers, listToolMcpServers, removeMcpServer, syncMcp, syncMcpServer } from "./core/mcp";
...
const menuCommands: MenuCommand[] = [
  ...
  { group: "Skill Presets", label: "Remove skill from preset", description: "Keep the skill, remove group membership.", argv: ["presets", "remove-skill", "$skill", "$from"] },
...
async function cmdSkills(argv: string[], global: GlobalOptions): Promise<void> {
  ...
  switch (sub) {
    ...
    case "unlink": {
      ...
      const result = await removeSkillAgentLink(name, tool);
      if (global.json) return printJson(result);
    ...
    case "sync-selected": {
      ...
      const result = [];
      for (const name of names) result.push(...await syncSkill(name, tool, mode));
      if (global.json) return printJson(result);
...
async function cmdPresets(argv: string[], global: GlobalOptions): Promise<void> {
  ...
  switch (sub) {
    ...
    case "remove-skill": {
      ...
      const result = await removeSkillFromPreset(skill, preset);
      if (global.json) return printJson(result);
      success(`Removed ${result.skill} from ${result.preset}. The managed skill was kept.`);
      break;
    }
    default:
      throw new UsageError("Usage: tm presets <list|apply|move|move-skill|remove-skill>");
  }
}
...
async function cmdMcp(argv: string[], global: GlobalOptions): Promise<void> {
  ...
  switch (sub) {
    ...
    case "sync-selected": {
      ...
      const result = [];
      for (const name of names) result.push(...await syncMcpServer(name, tool));
      if (global.json) return printJson(result);
...
function printToolMcpServers(result: Awaited<ReturnType<typeof listToolMcpServers>>): void {
  const rows = result.flatMap((item) => item.servers.map((server) => ({ tool: item.tool, name: server.name, transport: server.transport, endpoint: mcpEndpoint(server), credentials: Object.keys(server.transport === "http" ? server.headers : server.env).length, path: item.path })));
